export type WordRevealPalette = 'light' | 'dark';

export type RgbaColor = { r: number; g: number; b: number; a: number };

export const WORD_REVEAL_ACCENTS = {
  burgundy: {
    pale: { r: 180, g: 150, b: 150, a: 1 },
...
export function scaledFadeZone(base: number, wordCount: number) {
  return Math.max(base, Math.round(wordCount * 0.55));
}
